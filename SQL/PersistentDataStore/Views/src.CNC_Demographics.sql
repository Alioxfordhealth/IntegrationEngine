...
CREATE VIEW [src].[CNC_Demographics]
AS
WITH Ad
AS (
...
FROM mrr.CNC_tblPatient                            p
    ...
    LEFT OUTER JOIN mrr.CNC_tblLanguageValues      lv
        ON lv.Language_ID = p.First_Language_ID
    LEFT OUTER JOIN mrr.CNC_tblEthnicityValues     ev
        ON ev.Ethnicity_ID = p.Ethnicity_ID
    LEFT OUTER JOIN mrr.CNC_tblMaritalStatusValues msv
