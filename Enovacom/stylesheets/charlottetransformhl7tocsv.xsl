            ...
            <xsl:element name="row">
                <xsl:for-each select="$rootNode/PID">
                    ...
                    <xsl:for-each select="PID.5.2">
                        <xsl:variable name="var2_PID.5.2" select="."/>
                        <xsl:element name="firstname">
                            <xsl:value-of select="$var2_PID.5.2"/>
                        </xsl:element>
                    </xsl:for-each>
                </xsl:for-each>
                <xsl:for-each select="$rootNode/PID">
                    <xsl:variable name="var3_PID" select="."/>
                    <xsl:for-each select="PID.5.1">
                        <xsl:variable name="var4_PID.5.1" select="."/>
                        <xsl:element name="lastname">
                            <xsl:value-of select="$var4_PID.5.1"/>
                        </xsl:element>
                    </xsl:for-each>
                </xsl:for-each>
                <xsl:for-each select="$rootNode/PID">
                    ...
                    <xsl:for-each select="PID.3.1">
                        <xsl:variable name="var6_PID.3.1" select="."/>
                        <xsl:element name="id">
                            <xsl:value-of select="$var6_PID.3.1"/>
                        </xsl:element>
                    </xsl:for-each>
                </xsl:for-each>
            </xsl:element>
