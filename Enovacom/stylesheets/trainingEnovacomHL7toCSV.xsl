            ...
            <xsl:element name="row">
                ...
                <xsl:for-each select="$rootNode/PID">
                    ...
                    <xsl:for-each select="PID.5.1">
                        ...
                        <xsl:variable name="var6_minuscules">
                            <xsl:call-template name="minuscules">
                                <xsl:with-param name="string" select="$var5_PID.5.1"/>
                            </xsl:call-template>
                        </xsl:variable>
                        <xsl:element name="lastname">
                            <xsl:value-of select="$var6_minuscules"/>
                        </xsl:element>
                    </xsl:for-each>
                </xsl:for-each>
                <xsl:for-each select="$rootNode/PID">
                    ...
                    <xsl:for-each select="PID.3.1">
                        ...
                        <xsl:element name="id">
                            <xsl:value-of select="concat($prefix, $var8_PID.3.1)"/>
                        </xsl:element>
                    </xsl:for-each>
                </xsl:for-each>
            </xsl:element>
