...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<PID>
			...
			<PID.12.1/>
			<PID.13.1><xsl:value-of select="//TelephoneNumber/text()"/></PID.13.1>
			<PID.13.2><xsl:value-of select="//TelephoneUseCode/text()"/></PID.13.2>
			<PID.13.3><xsl:value-of select="//TelephoneType/text()"/></PID.13.3>
